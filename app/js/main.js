$(function (){
  ...
  $('.product-top__slider-link').on('click', function(){
    $('.popap').fadeIn();
    $('body').addClass('lock');
  });
  ...
  $(document).mouseup(function (e) { 
	var shopFltr = $('.shop-filter');
	if (e.target!=shopFltr[0]&&shopFltr.has(e.target).length === 0){
  ...
  $(document).mouseup(function (e) { 
	...
	if (e.target!=cartBox[0]&&cartBox.has(e.target).length === 0){
